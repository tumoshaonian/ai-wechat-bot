...
class Settings:
    ...
    def from_environment(cls) -> "Settings":
        ...
        harness_system_prompt += (
            ...
            "\n工具执行前有独立权限守卫：除窗口观察及确认交互外，工具会自动按实际参数向原用户申请一次确认。"
            "不要为同一工具再主动申请重复确认，不得尝试换工具、委派或脚本绕过；参数过长应拆分且说明目的。"
            "request_confirmation 仅用于额外的意图或替代方案确认，不能代替实际工具执行授权。"
            "需要意图确认时调用 request_confirmation，完整说明 action、target、effect；"
