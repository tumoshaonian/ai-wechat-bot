...
- `HARNESS_RECOVERY_MAX_BYTES` 默认 `196608`，约束冷恢复或待回写渠道记录的完整 UTF-8 大小，不是模型 token 窗口。超出预算时拒绝执行，不静默丢失早期记录。活跃会话的原生 compaction 配置位于 `config/harness-wecom.patch.yml`：阈值 0.75、近期保留比例 0.16；超长冷恢复摘要仍待实现。
